...
const PhoneInput = forwardRef<GetInputData, PhoneInputProps>(({
   ...
   return (
      <div className="flex flex-col font-tertiary mb-[24px]">
         ...
         <div>
            <select ref={selectionRef} className={`absolute w-[87px] rounded-[12px] py-[10px] px-[14px] border-p-light-gray border-1 outline-none ${inputTextStyle}`}>
               <option>+62</option>
            </select>
            <input ref={inputRef} onChange={onChangeText} type="tel" name="phone" placeholder={placeholder} className={`rounded-[12px] w-[100%] py-[10px] pl-[97px] pr-[14px] border-p-light-gray border-1 outline-none ${inputTextStyle}`} />
         </div>
      </div>
   )
})
